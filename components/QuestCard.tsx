...
export const QuestCard: React.FC<QuestCardProps> = ({ quest, playerLevel, isCompleted, isPending, canAfford = true, onAccept }) => {
  ...
  return (
    <div className={`relative transition-all duration-700 transform ${isTearing ? 'translate-y-20 opacity-0 rotate-6 scale-90' : ''}`}>
      ...
      <div className={`p-6 pt-8 rounded-sm border shadow-sm transition-all ${isCompleted ? 'bg-gray-100 border-gray-200 opacity-60 grayscale' :
        ...
        <div className="mb-4">
          ...
          {quest.description && quest.description.length > 40 && (
            <button
              ...
              className={`text-[10px] font-bold mt-1 opacity-60 hover:opacity-100 transition-opacity ${theme.text}`}
            >
              {isExpanded ? '收起 ∧' : '更多 ∨'}
            </button>
          )}
        ...
        <div className="flex justify-between items-end border-t border-black/5 pt-4 mt-2">
          ...
          <button
            ...
            onClick={handleTear}
            className={`px-4 py-2 rounded-lg text-[10px] font-bold uppercase transition-all ${isCompleted ? 'text-slate-400' :
